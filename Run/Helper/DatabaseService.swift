...
private extension DatabaseService {
    func getCollection<T: DictionaryConvertible>(for userId: String,
                                                 from ref: CollectionReference) async throws -> OrderedSet<T> {
...
extension DatabaseService: StopwatchToDatabaseServiceProtocol {
    ...
    func updateSneakers(for userId: String, on distance: Double) async throws {
        logger.log("\(#fileID) -> \(#function)")
        let querySnapshot = try await sneakersRef.whereField("userId",
                                                             isEqualTo: userId).whereField("isActive",
                                                            isEqualTo: true).getDocuments()
        let batch = db.batch()
        for document in querySnapshot.documents {
            let shoeRef = sneakersRef.document(document.documentID)
