...
extension UIImageView {
    func setImage(imageUrl: String, withCurvedCorners: Bool = true, completion: (() -> ())? = nil) {
        let processor = RoundCornerImageProcessor(cornerRadius: 12)
        self.kf.setImage(with: URL(string: imageUrl), options: withCurvedCorners ? [.transition(.fade(0.2)), .processor(processor)] : [.transition(.fade(0.2))]) { result in
            completion?()
        }
    }
    ...
    func setImageWithPlaceholder(imageUrl: String?,
        ...
        if let imageUrl = imageUrl, !imageUrl.trimm.isEmpty, imageUrl.isImage() {
            self.kf.setImage(with: URL(string: imageUrl), options: [.transition(.fade(0.2)), .fromMemoryCacheOrRefresh]) { [weak self] result in
    ...
    func setImageWithPlaceholderAndCompletionImage(imageUrl: String?,
        ...
        if let imageUrl = imageUrl, !imageUrl.trimm.isEmpty, imageUrl.isImage() {
            self.kf.setImage(with: URL(string: imageUrl), options: [.transition(.fade(0.2)), .fromMemoryCacheOrRefresh]) { result in
                switch result {
                case .failure:
                    completion?(nil)
