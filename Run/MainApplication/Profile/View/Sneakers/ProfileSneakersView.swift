...
final class ProfileSneakersView: UIView {
    ...
    private var dataSource = [Sneakers]()
    ...
    private lazy var collection: UICollectionView = {
        ...
        collection.register(ProfileSneakersCollectionViewCell.self,
                            forCellWithReuseIdentifier: ProfileSneakersCollectionViewCell.reuseIdentifier)
        return collection
    }()
    ...
    private func configureScrollingBehaviour() {
        let configuration = CollectionViewConfiguration(layoutType: .fixedSize(sizeValue:
                                            UIScreen.main.bounds.width - Constants.sliderWidthInset, lineSpacing: 0),
                                                        scrollingDirection: .horizontal)
        infiniteScrollingBehaviour = InfiniteScrollingBehaviour(withCollectionView: collection,
                                                                andData: dataSource,
                                                                delegate: self,
...
extension ProfileSneakersView: InfiniteScrollingBehaviourDelegate {
    func configuredCell(forItemAtIndexPath indexPath: IndexPath,
        ...
        guard let cellData = data as? Sneakers,
              let cell = collection.dequeueReusableCell(withReuseIdentifier:
                ProfileSneakersCollectionViewCell.reuseIdentifier, for: indexPath) as? ProfileSneakersCollectionViewCell
